...
import type { RfInvoiceItem, RfTax } from './types/rf';
...
function taxForLine(taxPercent: number): RfTax | undefined {
	if (!taxPercent || taxPercent <= 0) return undefined;
	return { type: 'percentage', amount: taxPercent };
}

export function buildRfLineItems(invoice: ClockifyInvoice): RfInvoiceItem[] {
	const currency = invoice.currency;
	const tax = taxForLine(invoice.taxPercent);
	const lines: RfInvoiceItem[] = [];
