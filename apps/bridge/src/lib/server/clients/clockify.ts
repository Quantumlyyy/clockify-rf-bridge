...
import type { ClockifyInvoice, ClockifyClientSummary, WorkspaceSettings } from '../types/clockify';
...
export async function getInstallToken(env: Env, workspaceId: string): Promise<string> {
	const token = await getEncryptedToken(env, workspaceId, 'clockify_install');
	if (!token) {
...
export async function getClients(
	opts: ClockifyClientOptions,
	workspaceId: string
): Promise<ClockifyClientSummary[]> {
	const url = `${apiBase(opts.backendUrl)}/v1/workspaces/${workspaceId}/clients`;
	const pageSize = 500;
	const clients: ClockifyClientSummary[] = [];
	let page = 1;

	while (true) {
		const batch = await clockifyFetch<ClockifyClientSummary[]>(
